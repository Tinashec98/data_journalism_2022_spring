...
**A1.** 126 E Burke St; 6 loans; Drew Holdings LLC, BRIX27, LLC, Abraham Ashton, HUB CO-OP, LLC, Ronin Properties LLC.

```{r}

zip_25401_loan_sample_2 <- read_csv("data/zip_25401_loan_sample_2.csv")

zip_25401_loan_sample_2%>%
  group_by(address)%>%
    summarize(count=n()
     ) %>%
  arrange(desc(count))
...
**A2.** It is suspicious because one of the directors at Drew Holdings LLC is Abraham Ashton. "Abraham Ashton" is listed in the data as a business at the same address as Drew Holdings LLC that also received a loan. "HUB CO-OP, LLC" and "HUB CO-OP LLC" appear under the same search result however the data set shows that they are two separate businesses that received different loans. Abraham Ashton is a director at Ronin Properties LLC and at BRIX27, LLC. Additionally, Robert Johnson is an agent at all of these businesses. Hence, it is possible that Ashton and Johnson registered multiple LLCs under their names so they could receive as many PPP loans as they could. 
...
wv_berkeley_loans <- lab_05%>%
  filter(project_county_name == "BERKELEY")
  
write_csv(wv_berkeley_loans, "data/wv_berkeley_loans.csv")

wv_berkeley_loans_clean <- read_csv("data/wv_berkeley_loans_clean.csv")

wv_berkeley_loans_clean%>%
  group_by(city)%>%
    summarize(count=n()
     ) %>%
  arrange(desc(count))

wv_berkeley_naics_codes <- wv_berkeley_loans_clean%>%
  left_join(naics_codes, by="naics_code")

wv_berkeley_naics_codes%>%
...
**A3.** The city with the highest loans was Martinsburg. The industry with the most loans is full service restaurants. 
...
**A4.**Restaurants experienced the biggest hit due to the pandemic hence the high amount of loans needed to keep them afloat. 
...
wv_martinsburg_loans <- wv_berkeley_naics_codes%>%
  filter(city == "Martinsburg", title == "Full-Service Restaurants")%>%
  
  group_by(name)%>%
    arrange(desc(amount))

wv_martinsburg_loans%>%
  filter(name == "COTTLE CHRISTI L LLC")
  

```

**A5.** The business with the highest approved loan amount is Cottle Christi L LLC. It was approved for $280,434. 94 jobs were retained. 
