...
```{r echo=FALSE}
census_api_key("4b8431442b60a8e17bdc0fd1d951762f5b318f5c", overwrite=TRUE)
```
...
**A3** Prince George's County, Montgomery County, Caroline County, Frederick County, Talbot County. 

```{r}

md_spanish_age_range <- get_acs(geography = "county",
              variables = c(spanishagerange = "B16007_004"),
              state = "MD",
              year = 2019)

md_total_age_range <- get_acs(geography = "county",
              variables = c(spanishpeaking = "B16007_002"),
              state = "MD",
              year = 2019)

md_percentage_population <- md_spanish_age_range %>%
  inner_join(md_total_age_range, by=c("GEOID", "NAME")) %>%
  rename(estimate_spanish_age_range=estimate.x, estimate_total_age_range=estimate.y) %>%
  mutate(percentage_population = ((estimate_spanish_age_range/estimate_total_age_range)*100)) %>%
...
**A4**Montgomery County and Prince George's County are in the metro D.C. Talbot County has a median age of over 50. Garrett County 
...
**A5** 60620 has a percentage Black population of about 96%. 

```{r}
total_population <- get_acs(geography = "zcta",
              variables = c(totalpopulation = "B02001_001"),
              year = 2019)

total_black_population <- get_acs(geography = "zcta",
              variables = c(totalblackpopulation = "B02001_003"),
              year = 2019)

percentage_black_population <- total_population %>%
  inner_join(total_black_population, by=c("GEOID", "NAME")) %>%
  rename(estimate_total_population=estimate.x, estimate_total_black_population=estimate.y) %>%
  mutate(percentage_population = ((estimate_total_black_population/estimate_total_population)*100)) %>%
  arrange(desc(percentage_population))
...
removed_by_zip <- top_zips%>%
  left_join(percentage_black_population, by=c('zip5'='GEOID'))%>%
  arrange(desc(total))
```
...
**A6** Nine of the zip codes are located in Chicago. One zip code is located in Atlanta. The two areas are the Black meccas of the country. The zip codes are also concentrated in the poorer areas of both cities. 

**Q7** What other columns from the PPP data could be useful in reporting on what happened to these removed loans?
**A7** The estimate population and estimated Black population columns. 
