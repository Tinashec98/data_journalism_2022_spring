...
**A1.** Illinois had the most loan applications from barber shops.15.4% of the undisbursed amounts were greater than 0. Illinois also has the most undisbursed applications. 
```{r}
barber_shops_ppp <- read_csv("data/barber_shops_ppp.csv")

barber_shops_state <- barber_shops_ppp %>%
    group_by(state) %>%
    summarise(count=n())%>%
    arrange(desc(count))

barber_shops_state_undisbursed <- barber_shops_ppp %>%
    filter(undisbursed_amount > 0) %>%
    group_by(state) %>%
    summarise(count=n())%>%
    arrange(desc(count))

together <- barber_shops_state %>%
  inner_join(barber_shops_state_undisbursed, by=c("state")) %>%
  rename(count = count.x, undisbursed = count.y )

write_csv(together %>% head(10), "barber_shops.csv") 
...
```{r}
together_percentages <- together %>% 
  head(10)

together_percentages <- together_percentages %>% 
  mutate(pct = undisbursed/count*100)

write_csv(together_percentages, "barber_shops_percentages.csv") 
```

**A2.** https://datawrapper.dwcdn.net/8dRZi/1/. This chart is different because it compares undisbursed loan amounts to total loan amounts and the percentage of undisbursed loan amounts in each state. 
...
**A3.**  https://datawrapper.dwcdn.net/Bz4sN/1/. Kabbage, Inc. received the most loans in the east coast and the south. 
