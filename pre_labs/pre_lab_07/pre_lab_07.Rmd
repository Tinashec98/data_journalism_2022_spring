...
**Answer** Teh table has two columns: sector and description. In the, sector column each value is assigned a number between 11 and 92. The description column lists out what exact industry is associated with the sector number. 
...
**Answer** There is code outlining what the NAICS Sectors table should look like. 
...
**Answer** Error in naics_url %>% read_html() : could not find function "%>%"
...
**Answer** There is a general map of what is nested in <html>, <head> and <body>.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** There is code describing the NAICS Sector table, information about style attribute, and a box model. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** List 6, list length of 6
...
**Answer** The first dataframe has data about conversion, reference month implemented, and date released. Dataframe 2 has information about the sector and its description. Dataframe 3 has information about the division and its description. Dataframe 4 has NAICS and SIC information. Dataframe 5 and 6 have information about positions, example value, field names and all possible values.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** A table with 21 rows and 2 columns appears. 
...
**Answer** The table that appears on the BLS' site. However, there's an extra row that doesn't have actual data. 
...
**Answer** The same table as in 17, standardized names, and a deleted 21st row. 
...
**Answer** There's a dataframe displaying the sector and description, akin to the table on the BLS' website.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** There is workforce statistics data. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** HTML details regarding <head> and <body> appear. 
...
**Answer** HTML code regarding <caption>, <thead>, <tbody> and <tfoot> - all specific instructions regarding how data will appear in the table. 
...
**Answer** The table appears as a dataframe. 
...
**Answer** The dataframe only shows "Employment, all employees (seasonally adjusted)" data that has been filtered out. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Workforce statistics tables for mining, quarryig, and oil and gas extraction. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Workforce statistics tables for the construction sector. 
...
**Answer** A list of the sectors appears. 
```{r}
...
**Answer** Nothing appears below the codeblock. However, list
_of_sectors appears under values in the environment tab. 
```{r}
...
**Answer** A list of sectors appears.
```{r}
...
**Answer** Error in sector_list : object 'sector_list' not found
...
**Answer** A list of prints 1 to 10. 
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer** A dataframe with sectors and descriptions. 
...
**Answer** The output shows that there are 20 rows in the table. 
...
**Answer** There are print numbers 1 to 20 in the naics_industry. 
...
**Answer** There is a dataframe with information about data series and Nov 21.
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** There is a dataframe with information about the sector and its description. 
```{r}
# Define url of page we want to scrape
...
**Answer** There is a dataframe with information about the sector, description and the sector URL. 
...
**Answer** There is a dataframe exactly like the previous one just with a sector_xpath_id column added. 
...
**Answer** A dataframe without the public administration sector. 
...
**Answer** There are 19 dataframes each with one row, one for each sector. 
...
**Answer** There is a list describing each row with its URL and the xpath id. 
```{r}
...
**Answer** There are 19 dataframes: one with 2 rows, 6 for each data series, two with 4 rows, 6 for each data series, three with 6 rows, 6 for each data series, three with 9 rows, 6 for each data series, and nine with 10 rows, 6 for each data series. 
...
**Answer** 19 dataframes with 1 row, 6 for each data series. 
...
**Answer** 19 dataframes, each with 1 row, 10 for each data series. 
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer** 19 dataframes, each with 1 row, 3 for each sector. 
...
**Answer** A dataframe appears with 3 columns: sector,descriptions, and nov_2021_employees. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Workforce statistics for mining, quarrying, and oil and gas extraction: NAICS 21
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Workforce statistics for Agriculture, Forestry, Fishing and Hunting: NAICS 11. 
...
**Answer** A cleaned dataframe without the agriculture sector appears. 
