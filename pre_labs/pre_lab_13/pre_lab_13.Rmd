...
**Answer** A dataframe with 5 columns and 51 rows.
```{r}
...
**Answer**Business that voted more for trump received more in loans.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer**Businesses that voted for Trump received more PPP money per employee.
```{r}
...
**Answer** A description of the correlation between businesses that voted for Trump and how their voting metrics influence how much loan money they get.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**The confidence interval has decreased.
```{r}
...
**Answer** Businesses that voted for Trump got more loan money than those that voted for Clinton.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**The t-value is closer to 1, meaning that the findings in the previous dataframe are somewhat reliable.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer**The t value is stronger, meaning that the data is reliable. 
```{r}
