...
**Answer**: 135,073. Yes the number is the same as the number derived from running separate code blocks. The conclusion is that the loans haven't been forgiven. 
...
**Answer** There is no data for October. 
```{r}
ppp_maryland_loans %>%
...
**Answer** The smallest loan is $6. This is suspicious because why would someone apply for a $6 loan?
```{r}
...
**Answer** Stuart Jones LLC has two entries. There are many different spellings of "Arnold." Some zip codes have add-ons while others don't. 
...
**Answer** The function adds an "x" to the column name. The code just changes the 1_id column name.
...
**Answer** The column name has now changed to just "id."
...
**Answer** The error says "Problem with `summarise()` column `total_amount`." this means that the amount column is listed as a character when it's supposed to be listed in <dbl> format. 
...
**Answer** 2 duplicate rows. Stuart Jones LLC. 
...
**Answer** 5 rows.The table now only lists Stuart Jones LLC once showing that the business submitted only one application. 
...
**Answer** The zip code listings are now uniform. 
...
**Answer** 2 misspellings remain. The data set is unchanged. 
...
**Answer** No misspellings of "Arnold" remain. The data in the table is now cleaned up. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
